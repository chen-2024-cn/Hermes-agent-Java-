# install-hermes.ps1
# 一键把 sk-hermes 安装为全局命令 `hermes`，并注册文件夹右键菜单「在此处使用 Hermes 聊天」
...
Set-ItemProperty -Path $keyPath -Name '(default)' -Value '在此处使用 Hermes 聊天'
New-Item -Path "$keyPath\command" -Force | Out-Null
# %V = 当前文件夹路径（资源管理器背景右键专用变量）
Set-ItemProperty -Path "$keyPath\command" -Name '(default)' `
    -Value ('cmd /k cd /d "%V" && hermes')
Write-Host "[3/3] 已注册文件夹右键菜单「在此处使用 Hermes 聊天」" -ForegroundColor Green

Write-Host ''
Write-Host '完成！新开一个终端，效果如下：' -ForegroundColor Cyan
Write-Host '  cd <任意项目文件夹>'
Write-Host '  hermes          # 直接进入对话（模型自动以当前目录为项目根）'
Write-Host '  hermes --help   # 查看全部子命令'
